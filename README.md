...
1. **한 줄 요약**  
   - Elasticsearch 도입으로 성능 % 개선
   - 대규모 데이터에서도 비교적 안정적인 성능 유지
   

2. **도입 배경**  
   - 데이터가 증가할수록 검색 할때 DB의 Full-Table Scan으로 인해 성능 저하 발생

3. **기술적 선택지**  
   1. **인덱스**
      - 정확한 값 검색 성능은 개선되지만, 인덱스 길이 제한으로 인한 Full-Text Index 사용 어려움
      - '텍스트 포함 검색'은 지원하지 않거나 비효율적임
      - 쓰기 성능 저하
   2. **Elasticsearch**
      - 단어 포함 검색이 가능하고, 역색인 구조를 사용하여 대규모 데이터 처리에 적합함
      - 유사 검색, 동의어 검색등 고급 검색 가능    

   **결론:** Elasticsearch를 도입하여 성능 및 검색 확장성 개선
